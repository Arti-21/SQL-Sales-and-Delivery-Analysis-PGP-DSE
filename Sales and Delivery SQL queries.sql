...
-- Update the DaysTakenForDelivery column using a JOIN and DATEDIFF
SET SQL_SAFE_UPDATES = 0;
SET DATEFORMAT 34;
ALTER TABLE orders_dimen MODIFY COLUMN Order_Date text;
SELECT FORMAT(Order_Date,'dd-mm-yyyy') as newdate from orders_dimen;
SELECT STR_TO_DATE(Order_Date,'%d-%m-%Y') from orders_dimen;
SELECT STR_TO_DATE(Ship_Date,'%d-%m-%Y') from shipping_dimen;
alter table orders_dimen ADD Date_Ordered date;
update orders_dimen set Date_Ordered = STR_TO_DATE(Order_Date,'%d-%m-%Y');

select case when 

UPDATE orders_dimen AS o
JOIN shipping_dimen AS s ON o.Order_ID = s.Order_ID
